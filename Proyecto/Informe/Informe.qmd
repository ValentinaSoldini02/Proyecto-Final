...
datos<-read_csv(here("Proyecto","Datos","houses.csv"))
...
Por otro lado, a los datos duplicados se eliminaron teniendo en cuenta que las coincidencias únicas que tienen las filas, ya que no tenemos una fecha de snapshot de Mercado Libre.xnmsxnlks
...
ggplot(promedios3, aes(x = fct_reorder(Municipio, promedio_precio_m2), y = promedio_precio_m2, fill = promedio_precio_m2)) +
  ...
  scale_fill_continuous(type= "gradient", name= "Precio por m2") +
...
ggplot(datos_filtrados, aes(x = año_de_const, y = precio_m2, color = precio_m2)) +
  ...
  scale_color_continuous(type= "gradient", name = "Precio por m2") +
...
Este patrón es consistente con la idea de que las propiedades más nuevas tienen características y otras comodidades, mejor infraestructura y probablemente esten ubicadas en lugares mas deseables.
...
ggplot(grupo_disposicion, aes(x = fct_reorder(disposicion, Cantidad), y = Cantidad, fill = Cantidad)) +
  geom_bar(stat = "identity") +
  scale_fill_continuous(type = "gradient", name = "Cantidad") +
...
ggplot(data = datos_3) +
  ...
  scale_fill_discrete( name = "Precio por m2") +
...
ggplot(promedios2, aes(x = fct_reorder(cant_de_est, promedio_precio_m2), y = promedio_precio_m2, fill = promedio_precio_m2)) +
  geom_bar(stat = "identity") +
  scale_fill_continuous(type ="gradient", name = "Precio por m2") +  # Utilizar la paleta de colores RdYlBu
...
ggplot(datos_2, aes(x = Gastos_Comunes, y = precio_m2, color = precio_m2)) +
  ...
  scale_color_continuous(type= 'gradient', name = "Precio por m2") +  # Aplica la paleta de colores RdYlBu y define la leyenda
