...
datos <- read_excel(here("Proyecto","Datos","datos_limpios.xlsx"))
datos
...
datos_mapa <- read.csv(here("Proyecto","Shiny", "Miapp", "datoss.csv"))
shape <- st_read(here("Proyecto","Shiny","Miapp", "sig_municipios.shp"))
...
ui <- fluidPage(     # Definimos una página en blanco
  ...
  sidebarLayout( # Definimos la estructura
    sidebarPanel(# Colocar los selectInput
      ...
      # ESTRUCTURA DE GASTOS COMUNES POR MUNICIPIO
      conditionalPanel( # Panel condicional
        condition = "input.tabselected == 4", # Le asociamos el valor 4
# Este input va a estar asociado al gráfico de todos los municipios.
...
# Distintos ejes X para la reactividad, donde estaremos eligiendo un barrio de todos los que hay
     ...
     # ESTRUCTURA DE LOS MOSAICOS
      conditionalPanel( # Panel Condicional
        condition = "input.tabselected == 5", # Le asociamos el valor 5
        selectInput("x_axis_bernoulli", "Seleccione el eje X para los gráficos de Bernoulli:", # Distintos ejes X para la reactividad
                    choices = list( # Opciones a elegir
      ...
      # OUTPUT DE PRECIO_M2 POR MUNICIPIO
      conditionalPanel( # Panel Condicional
      ...
      tabsetPanel(id = "tabselected",  # Pestañas posibles (cuatro)
                  tabPanel("Mapa",  # Pestaña del mapa
                          h2("Gráfico de Mapa"), # Título
                           leafletOutput("mapa", width="100%", height= "600px"), # Parámetros relacionados con la imagen del mapa
                  ...
                  tabPanel("Municipio GC", # Pestaña de Gastos Comunes por Municipio
                       ...
                       vale = 4 # Valor asociado
                  ),
                  tabPanel("Mosaicos", # Pestaña de Mosaicos
                           h2("Gráfico de Bernoulli"), # Título
...
observeEvent(input$update_municipio_GC, { # Gráfico asociado a el input de todos los municipios
  output$municipioPlot1_GC <- renderPlot({ # Gráfico de los municipios
    ...
    ggplot(datos, aes_string(x = x_axis, y = "Gastos_Comunes", fill = "Municipio")) + # Datos, ejes y color
      ...
      # Nombre de los ejes y colores asociados
    ...
    # Tamaño y eje de los textos del eje X
  })
  
  output$singleMunicipioPlot_GC <- renderPlot({ # Municipios especifico
    x_axis <- input$x_axis__GC_municipio # Distintos ejes X
 ...
 # Nombre de los ejes
  ...
  observeEvent(input$update_bernoulli, { # Input asociado
    output$bernoulliPlot <- renderPlot({ # Output de salida
      ...
      ggplot(data = datos) + # Datos
        geom_mosaic(aes_string(x = paste0("product(precio_m2_rec, ", x_axis, ")"), fill= "precio_m2_rec")) + # Gráfico de mosaico, donde en tenemos distintos ejes X categoricos (dos categorías) y en ele eje Y tenemos dos categorías que son el precio_m2 mayor o menor al promedio de precio_m2
        labs(title='Comparación del precio por metro cuadrado y el número de piso', x = x_axis, y = "Precio por metro cuadrado")+ # Nombre de los ejes
...
observeEvent(input$update_municipio, { # Gráfico asociado a el input de todos los municipios
  ...
  output$zonaPlot <- renderPlot({ # Barrio especifico
    ...
    datos_filtrados_zona <- datos[datos$zona == selected_zona, ] 
    # Nos quedamos solo con los datos de ese barrio
    ...
    if(input$change_mapa== "mapa1"){  # Si elegimos mapa1
      leaflet() %>% addTiles() %>%    # Agrega los tiles del mapa base
        ...
        addCircles( data= datos_mapa,         # Datos
                    ...
                    color= ~pal(Municipio), label= ~Municipio, # Color
                    fillOpacity = 0.5) %>%    # Opacidad
        addPolygons(data= shape,  # Agregar poligonos al mapa
                    fillColor = ~pal(MUNICIPIO), # Color
                    weight = 0.5, # Grosor
                    label= ~MUNICIPIO) %>% # Etiquetas
        addLegend(data= datos_mapa, "bottomleft", pal= pal, # Leyenda
                  values= ~Municipio,  # Valores 
                  title= "Municipios", # Título 
                  opacity= 1,          # Opacidad
                  group= "Leyenda"     # Grupos de la leyenda
        ) %>%
        addLayersControl( # Control de capas
                         overlayGroups = c("Leyenda"),# Grupos de capas
                         options= layersControlOptions(collapsed=TRUE))
                         # Opciones para controlar las capas
      ...
      leaflet() %>% addTiles() %>% # Agrega los tiles del mapa base
        ...
        addPolygons(data= shape,              # Agregar poligonos 
                    ...
                    weight = 0.5,                  # Grosor
                    label= ~medias,        # Etiquetas en base a medias
                    popup= ~MUNICIPIO) %>% # Información al hacer click
        
        addLegend(data=shape, "bottomleft",  # Leyenda y datos
                  pal=color_pal,             # Palet de colores
                  values= ~total,            # Valores
                  title="Cantidad",          # Título
                  opacity=1,                 # Opacidad
                  group="Leyenda") %>%       # Grupos
        addLayersControl( # Agrega control de capas al mapa
          overlayGroups = c("Leyenda"), # Grupos de capas para control
          options=layersControlOptions(collapsed=TRUE)) 
