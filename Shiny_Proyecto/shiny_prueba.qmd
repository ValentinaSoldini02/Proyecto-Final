  ...
  sidebarLayout(
    sidebarPanel(
      conditionalPanel(
        ...
        selectInput("x_axis_datos1", "Seleccione el eje X:", 
                    choices = list("Baños" = "baños", 
                                  "Habitaciones" = "cant_habitaciones",                                    #"Disposición" = "disposicion",
                                   "Cuartos" = "cant_cuartos",
                                   "Garages" = "cant_de_est")),
        actionButton("update_datos1", "Actualizar Gráficos")
      ),
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel(
        condition = "input.tabselected == 4",
        selectInput("x_axis_gastos", "Seleccione el eje X:", 
                    choices = list("Municipio" = "Municipio", 
                                   "Tipo de Propiedad" = "tipo_prop", 
                                   "Condición" = "Condicion")),
        actionButton("update_gastos", "Actualizar Gráficos")
...
server <- function(input, output, session) {
  ...
  observeEvent(input$update_gastos, {
    output$gastosPlot1 <- renderPlot({
      ggplot(datos, aes(x = Municipio, y = Gastos_Comunes, fill = Municipio)) + 
        geom_bar(stat="identity") + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1))
    })
    
    output$gastosPlot2 <- renderPlot({
      ggplot(datos, aes(x = tipo_prop, y = Gastos_Comunes, fill = Municipio)) + 
        geom_bar(stat="identity") + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1))
    })
    
    output$gastosPlot3 <- renderPlot({
      ggplot(datos, aes(x = Condicion, y = Gastos_Comunes, fill = Municipio)) + 
        geom_bar(stat="identity") + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1))
    })
  })
    ...
    mainPanel(
      tabsetPanel(id = "tabselected",
        tabPanel("Datos1",
                 h2("Gráfico de Datos1"),
                 plotOutput("barPlot"),
                 plotOutput("newPlot"),  # Nuevo gráfico
                 value = 1
        ),
        tabPanel("Datos2",
                 h2("Gráfico de Datos2"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$update_datos1, {
    ...
    output$newPlot <- renderPlot({
      x_axis <- input$x_axis_datos1
      datos <- datos %>% select(all_of(c(x_axis, "precio")))
      ggplot(datos, aes_string(x = x_axis, y = "precio")) + 
        geom_boxplot() +
        labs(x = x_axis, y = "Precio") +
        theme(axis.text.x = element_text(angle = 90, hjust = 1))
    })
...
server <- function(input, output, session) {
  ...
  observeEvent(input$update_datos1, {
    output$barPlot <- renderPlot({
      ...
      p1 <- ggplot(datos, aes_string(x = x_axis)) + 
        geom_bar() +
        scale_x_discrete(breaks = datos_resumidos[[x_axis]]) +  # Cambiado a scale_x_discrete
        labs(x = x_axis, y = "Cantidad")
      p2 <- ggplot(datos_resumidos, aes_string(x = x_axis, y = "media_precio")) + 
        geom_bar(stat = "identity") +
        scale_x_discrete(breaks = datos_resumidos[[x_axis]]) +  # Cambiado a scale_x_discrete
...
server <- function(input, output, session) {
  ...
  observeEvent(input$update_gastos, {
    output$gastosPlot1 <- renderPlot({
      ggplot(datos, aes(x = Municipio, y = Gastos_Comunes)) + 
        geom_bar(stat="identity") + 
        theme(axis.text.x = element_text(angle = 90, hjust = 1))
    })
    
    # output$gastosPlot2 <- renderPlot({
    #   ggplot(datos, aes(x = tipo_prop, y = Gastos_Comunes)) + 
    #     geom_bar(stat="identity") + 
    #     theme(axis.text.x = element_text(angle = 90, hjust = 1))
    # })
    
    # output$gastosPlot3 <- renderPlot({
    #   ggplot(datos, aes(x = Condicion, y = Gastos_Comunes)) + 
    #     geom_bar(stat="identity") + 
    #     theme(axis.text.x = element_text(angle = 90, hjust = 1))
    # })
  })
...
# Colores personalizados para Municipios
colores_personalizados <- c(
  "Municipio_A" = "red",  "Municipio_B" = "blue",  "Municipio_C" = "green",                   
  "Municipio_D" = "orange"
)
