...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      conditionalPanel(
        condition = "input.tabselected == 1",
        selectInput("x_axis_basicos", "Seleccione el eje X para los gráficos básicos:",
                    choices = list(
                      "Municipio" = "Municipio",
                      "Tipo de Propiedad" = "tipo_prop",
                      "Condición" = "Condicion")),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$update_municipio, {
    output$municipioPlot1 <- renderPlot({
      x_axis <- input$x_axis_municipio
      datos %>% 
        ggplot(aes_string(x = x_axis, fill = x_axis)) +
        geom_bar(position = "dodge") +
        scale_fill_manual(values = colores_personalizados) +
        labs(x = x_axis, y = "Frecuencia", fill = x_axis) +
...
server <- function(input, output, session) {
  ...
  observe({
    output$bernoulliPlot <- renderPlot({
      dum <- datos %>%
        select("vista_al_mar", "vivienda_social", "Barbecue", "Living_Room", "Closet", "Patio", "Pool", "Garage", 
               "Pet", "coneccion_cas", "Playroom", "Duplex", "calefaccion", "WiFi", "zona_barbacoa", "Gym", 
               "balcon", "deposito", "Air_Conditioning", "jardin", "losa_radiante", "Solarium", "amueblado")
