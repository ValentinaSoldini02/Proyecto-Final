...
Estas preguntas guiarán nuestro análisis para comprender mejor los factores que afectan los precios en el mercado inmobiliario y proporcionar una herramienta útil para los distintos agentes, ya sean compradores, vendedores, inversores y agentes inmobiliarios. La variable de interés de este proyecto es *precio_m2*
...
```{r}
#| fig-cap: "Qué Municipio le corresponde a cada zona"
...
# Función para insertar saltos de línea después de cada tres barrios
insert_line_breaks <- function(text, n = 3) {
  parts <- str_split(text, ",\\s*")[[1]]
  text_with_breaks <- paste(sapply(seq_along(parts), function(i) {
    if (i %% n == 0 && i != length(parts)) {
      return(paste(parts[i], "\n"))
    } else {
      return(parts[i])
    }
  }), collapse = ", ")
...
# Aplicar la función a la columna Columna2
tabla_municipio$Columna2 <- sapply(tabla_municipio$Columna2, insert_line_breaks)

# Generar la tabla con gt y ajustar el ancho
Municipios <- gt(tabla_municipio) |>
  ...
  tab_style(
    style = cell_text(size = "small", align = "justify"),
...
# Mostrar la tabla
gtsave(Municipios, here::here("output/tabla_municipios.png"))

knitr::include_graphics(here::here("output/tabla_municipios.png"))
