...
datos_sub <- datos_2  |> 
  select(-direccion, - losa_radiante, -Apartments_per_Floor, -baños, -cant_de_est, -Updated, -zona)
names(datos_sub)
```
...
modelo <- lm(precio_m2 ~. -losa_radiante - año_de_const - cant_de_est - baños -Apartments_per_Floor -cant_cuartos - cant_de_piso -direccion -zona, data = datos_2)
summary(modelo)
vif(modelo)
datos_sub
```
...
# este grafico nos muestra que las predicciones no son precisas.


resi <- testing$precio_m2 - pred


ggplot(data.frame(Residuals = resi), aes(x = Residuals)) +
  geom_histogram(binwidth = 10, fill = "blue", color = "black") +
  labs(x = "Reciduos", y = "frecuencia") +
  theme_minimal()
```
...
# Ajusto un ´arbol de clasificaci´on sin podar
tree_ab <- rpart( precio_m2~ Municipio + WiFi,
data = train.set)
