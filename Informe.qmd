...
```{r}
ggplot(datos_2, aes(x = calefaccion, y = precio_m2)) +
...
```{r}
ggplot(datos_2, aes(x = vista_al_mar, y = precio_m2)) +
  geom_bar(stat = "identity") +
...
En estos graficos lo que se trato de hacer es como algunas variables influyen en el precio por metros cuadrados. Como se puede observar con mucha imaginacion que las propiedades que estan constuidas mas recientemente su precio es mas alto.

```{r}
datos_2 <- datos_2 %>%
  mutate(tipo_prop = case_when(
    tipo_prop == "Apartament" ~ "Apartamento",
    tipo_prop == "Commercial" ~ "Comercial",
    tipo_prop == "House" ~ "Casa",
    tipo_prop == "Office" ~ "Oficina",
    TRUE ~ "otros"
  ))

ggplot(datos_2, aes(x = tipo_prop, y = precio_m2)) +
  geom_bar(stat = "identity") +
  labs(x = "Tipo de propiedad", y = "Impacto en el precio por m2") +
  ggtitle("Impacto del tipo de propiedad en el precio por m2") +
  theme_minimal()
```
...
>>>>>>> 42b0dfab3096a4582d7ca6b0f15261bc95354837
...
Vemos que en bienes como lo son: Aire Acondicionado, Amueblamiento, Balcon,  Barbacoa, Calefacción, Closet, Garage, Gimnasio, Jardin, Living, Losa Radiante, Habitación de juegos, Cuarto de Bronceado, Piscina, Vista al Mar, Vivienda Social, Wifi, Barbacoa ocurre que el precio es más alto al tener que al no tener. 
...
```
(Probablemente quitar este gráfico)
...
Se realizará un análisis exploratorio con el objetivo de identificar las variables que tienen mayor impacto en el precio por metro cuadrado. Para ello, se elaboraron graficos lineales que comparan distintas variables con el precio por metro cuadrado, permitiendo observar cómo cada una de estas variables agecta el precio de manera específica. Este análisis se ha realizado con casi todas las variables disponibles en los datos, sin embargo, en el informe solo se presentan algunos de los resultados más relevenates.

El gráfico a continuación muestra como afecta la contidad de baños en el precio por metro cuadrado.Se observa que las propiedades con un solo baño presentan un precio por metro cuadrado más alto. Esto sugiere que la cantidad de baños no influye significativamente en la determinación del precio de la propiedad. Sin embargo, se reconoce que esta conclusión puede no refelejarse a la realidad. Este resultado podría deberse a una posible falla en los datos o algún otro factor no considerado en el análisis.
...
Se aplicó el mismo proceso de categorización d¿utilizado para los baños a las propiedades en función de la cantidad de estacionamientos. En este caso, se excluyeron del análisis las propiedades que no tienen disponibilidad de estacionamientos, con el objetivo de comparar únicamente aquellas que sí cuentan con estacionamientos. Los resultados despuestran que la cantidad de estacionamientos influye significativamente en la determinación del precio por metro cuadrado.

```{r}
datos_2 <- datos_2 %>%
  filter(cant_de_est != 0) %>%
  mutate(cant_de_est = case_when(
    cant_de_est == 1 ~ "1",
    cant_de_est == 2 ~ "2",
    TRUE ~ "más de 3"
...
Vista al Mar: Similar a lo observado con los baños, las propiedades con vista al mar generalemnte tienen un precio por metro cuadrado más alto en comparación con las demás. Sin embargo, esta relación no necesariamente refelja la realidad del mercado.
