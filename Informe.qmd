...
La idea ahora es tratar de recodificar siertas variables para que los datos sean mas flexibles. La idea es agarrar ciertas variables como baños, cantidad de estacionamientos, catidad de pisos y recodificarlas.
...
ggplot(datos_2, aes(x = baños, y = mean(precio_m2))) +
  geom_bar(stat = "identity") +
...
ggplot(datos_2, aes(x = cant_de_est, y = precio_m2)) +
  geom_bar(stat = "identity") +
...
```{r}
ggplot(datos_2, aes(x = Municipio, y = precio_m2)) +
  geom_bar(stat = "identity") +
...
datos_2 <- datos_2 %>% 
  filter(precio_m2 < 400 & Gastos_Comunes < 200000)
