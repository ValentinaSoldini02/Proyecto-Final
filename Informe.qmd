...
# Función para reemplazar valores NA en 'Condicion' con 'Good condicion'
reemplazar_na_condicion <- function(datos_2) {
  ...
  datos_2$condicion[is.na(datos_2$condicion)] <- "Good condicion"
  return(datos_2)
}
#
...
En cuanto a la disposición de los inmuebles, la mayoría de las propiedades en alquiler están situadas al frente. Al analizar los precios promedio relacionados con la disposición de los inmuebles, encontramos que la mayoría tienen un precio similar, rondando los 500 dólares. Sin embargo, los inmuebles ubicados al frente presentan un precio significativamente más alto, alcanzando los 1500 dólares.
...
datos_sub <- datos_2  |> 
  select(-direccion, -baños, -cant_de_est, -zona, -cant_cuartos, -cant_de_piso, Air_condicioning, -losa_radiante )
names(datos_sub)
```
...
library(rpart.plot)
tree_ab <- rpart(precio_m2 ~ Municipio + condicion,
                 data = train.set,
