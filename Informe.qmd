...
datos_2 <- datos_2 %>%
  mutate(zona = case_when(
    zona %in% c("Carrasco Barrios con Seguridad") ~ "Carrasco",
    zona %in% c("Carrasco Este") ~ "Carrasco",
    zona %in% c("Golf") ~ "Punta Carretas",
    zona %in% c("Parque Miramar") ~ "Carrasco",
    zona %in% c("Peñarol Lavalleja") ~ "Peñarol",
    zona %in% c("Pocitos Nuevo") ~ "Pocitos",
    zona %in% c("Prado Nueva Savona") ~ "Prado",
    zona %in% c("Puerto Buceo") ~ "Buceo",
    zona %in% c("Villa Biarritz") ~ "Punta Carretas",
  ))
...
```
Por lo tanto, las propiedades más antiguas tienden a ser más económicas, mientras que las propiedades más recientes tienden a ser más caras. Este patrón es consistente con la idea de que las propiedades más nuevas tienen características y otras comodidades, mejor infraestructura y probablemnete esten ubicadas en lugares mas deseables.
...
# Crear el gráfico
ggplot(promedios6, aes(x = fct_reorder(tipo_prop,promedio_precio_m2) y = promedio_precio_m2)) +
  geom_bar(stat = "identity") +
...
Pregunta: ¿Cómo se distribuye el precio promedio según las condiciones de los alquileres?
...
ggplot(grupo_condicion, aes(x = fct_reorder(condicion, promedio_precio_m2), y = promedio_precio_m2)) +
  geom_bar(stat = "identity") +
...
Con el objetivo de identificar si las variables del modelo explican correctamente la variable de respuesta, en este caso *precio_m2*, se decidió hacer una exploración con ciertas variables. Para ello, se elaboraron gráficos lineales que anteriormente estan precentados que comparan distintas variables con el precio por metro cuadrado, permitiendo observar cómo cada una de estas afecta el precio de manera específica. Este análisis se ha realizado con casi todas las variables disponibles en los datos.

Una vez hecho este analisis, se paso a crear el modelo de regrecion lineal. Este modelo se creo a partir de las variables que mas afectaba el cambio en el precio por metro cuadrado. Es asi, que se eligieron las variables: *Municipio*,  *condicion*, *baños*, *año_de_const*, *tipo_prop*, *vista_al_mar*, *cant_de_est*, *nro_de_piso*, a pesar de que se redujo la cantidad de variables comparando con todas las variables que hay en el set de datos, se interpretó que asi se iba a poder ver un poco mejor que nos quieren mostrar las diferentes variables.

*precio_m2* = $\beta0$ + *Municipio*$\beta1$ +  *condicion*$\beta2$ + *baños*$\beta3$ + *año_de_const*$\beta4$ + *tipo_prop*$\beta5$ + *vista_al_mar*$\beta6$ + *cant_de_est*$\beta7$ + *nro_de_piso*$\beta8$
...
Por lo que se puede observar es que casi todos los coeficientes nos dan un p-valor mayor a 0.05 porque lo que significa que no son significativas al modelo. 

Luego por otro lado, analizando el $R^2$ nos podemos dar cuenta que las variables explicativas no explican correctamente a la variable de respuesta ya que el $R^2$ dio muy bajo
...
Que la significancia no se cumpla es decir que las variables no sean significativas y que el $R^2$ sea tan chico que las variables explicatibas no expliquen en absoluto a la variable de respiesta, es una mala señal. 

Por ende, a continuacion se van a evaluar los cuatros supuetsos para identificar si un modelo predice bien o no.
...
La raíz cuadrada del error cuadrático medio indica, en promedio, que las predicciones estan desviadas por 15.90 dólares del valor real del precio por metro cuadrado, lo cual significa que las predicciones estan bastante mal.
...
A este se dio por hecho debido a que la forma en que se iba a mostrar era inleible.
...
Debido a que casi ningún supuesto se cumplio, optampos por utilizar otros metodos de prediccion, que son el método de árboles y bosques:

## Árboles de decisión:

Los modelos basados en árboles consisten en una secuancia de particiones anidadas que dividen el espacio de los predictores donde en cada partición se usa un modelo para predecir la respuesta.
...
El árbol de decisión muestra que el año de construcción y la disposición de las propiedades son factores importantes para predecir el precio por metro cuadrado. Las propiedades más recientes tienden a tener un precio más alto, y la disposición también juega un papel significativo, especialmente en las propiedades más nuevas. 
...
Las observaciones dispersas alrededor de la línea roja indica que hay cierta variabilidad entre los valores originales y las predicciones. Esta predicción no es tan mala ya que la mayoria de los datos se encuentran bastante cerca de la línea roja. También se aprecia que las observaviones siguen la misma tendencia que dicha línea, esto significa que a medida que crecen las variables originales las predicciones también, por lo que es una buena señal.
...
ggplot(results, aes(x = Actual, y = Predicciones)) +
  ...
  theme_minimal()
```
Como en el gráfico anterior se muetsra la comparación entre las variables originales y las variables predichas usando el método de random forest, pero esta vez aplicándolo al conjunto de datos "testing" que son un 70% de los datos.

Con respecto a la dispersión alrededor de la línea roja, en comparación al gráfico anterior, usando el conjunto de entrenamiento, se observa una mayor disperción de las observaciones alrededor de la línea roja. Esto sugiere que el modelo tiene más disficultad para predecir correctamente.

La mayor dispersión y el alejamiento de algunos puntos de la líena roja indican que el modelo no está generalizado bien a datos nuevos y desconocidos. Este es un indicio de que el modelo podria estar sobreajustando al conjunto de datos de entrenamiento.
