...
# Visualización
Pregunta 1) ¿Cambia el precio con respecto al municipio en que se ecuestre la propiedad?
```{r}
...
Pregunta 7) ¿Como se distribuyen los gastos comúnes por Municipio?
```{r}
...
# Filtrar atipicos
grupo_piso_1 <-grupo_piso_1 %>% filter(cant_de_piso > 0 & cant_de_piso < 16)


datos <- datos  %>% filter(cant_de_piso > 0 & cant_de_piso < 16)


# Gráfico
ggplot(datos, aes(x = cant_de_piso)) +  # Datos y ejes
   geom_bar() +
    scale_x_continuous(breaks =   datos$cant_de_piso) 
...
Vemos que en general, lo más común es que los inmuebles no tengan garage o que tengan 1 solo, luego el tener 2 o 3 es menos común. De vuelta, se cumple que a más garages tenga un inmueble, más alto sera el precio.
...
Parece haber una relación clara sobre la relación entre metros cuadrados y precio, el precio va a ser mayor conforme sean mayores los metros cuadrados.
