...
Mosaico con los datos de 0  y 1
```{r}
...
datos_3 %>% filter(precio_m2_rec != "Precio m2 menor 13")

max(datos_3$precio_m2)
datos_3
...
# datos_4 <- datos_2 %>%
#   mutate(precio_m2_rec = case_when(
#     (precio_m2 >= 0 & precio_m2 < 5) ~ "Precio m2 entre 0 y 5",
#     (precio_m2 >= 5 & precio_m2 < 10) ~ "Precio m2 entre 5 y 10",
#     (precio_m2 >= 10 & precio_m2 < 15) ~ "Precio m2 entre 10 y 15",
#     (precio_m2 >= 15 & precio_m2 < 20) ~ "Precio m2 entre 15 y 20",
#   ))
# 
# datos_4
...
ggplot(data = datos_4) +
 geom_mosaic(aes(x = product(precio_m2_rec, losa_radiante), fill= precio_m2_rec)) +
 labs(title='Comparación del precio por metro cuadrado y el número de piso', x = "x", y = "Precio por metro cuadrado")+
 theme(legend.position = "bottom")   



colnames(dum)
# Condicionantes: vista_al_mar, Living_Room, Pool, Gym, Air_Conditioning, Solarium, Closet, vivienda_social (negativamtente), Garage (poco), Playroom,  balcon (poco), jardin, amueblado, Barbecue,   Patio (un poco si pero negativo), Pet (un poco si pero negativo), Duplex (negativamente),  zona_barbacoa, deposito (negativamente),  losa_radiante

# No Condicionantes: conexion_cas, WiFi,




# 
# [1] "vista_al_mar"     "vivienda_social"  "Barbecue"        
#  [4] "Living_Room"      "Closet"           "Patio"           
#  [7] "Pool"             "Garage"           "Pet"             
# [10] "coneccion_cas"    "Playroom"         "Duplex"          
# [13] "calefaccion"      "WiFi"             "zona_barbacoa"   
# [16] "Gym"              "balcon"           "deposito"        
# [19] "Air_Conditioning" "jardin"           "losa_radiante"   
# [22] "Solarium"         "amueblado"   




# Elejidas: conexion_cas, WiFi, Air_Conditioning, Pool, Gym, Duplex, deposito

# No condicionan
ggplot(data = datos_4) +
 geom_mosaic(aes(x = product(precio_m2_rec, conexion_gas), fill= precio_m2_rec)) +
 labs(title='Comparación del precio por metro cuadrado y el número de piso', x = "x", y = "Precio por metro cuadrado")+
 theme(legend.position = "bottom")   


ggplot(data = datos_4) +
 geom_mosaic(aes(x = product(precio_m2_rec, WiFi), fill= precio_m2_rec)) +
 labs(title='Comparación del precio por metro cuadrado y el número de piso', x = "x", y = "Precio por metro cuadrado")+
 theme(legend.position = "bottom")   
...
ggplot(data = datos_4) +
 geom_mosaic(aes(x = product(precio_m2_rec, Duplex), fill= precio_m2_rec)) +
 labs(title='Comparación del precio por metro cuadrado y el número de piso', x = "x", y = "Precio por metro cuadrado")+
 theme(legend.position = "bottom")   


ggplot(data = datos_4) +
 geom_mosaic(aes(x = product(precio_m2_rec, Pet), fill= precio_m2_rec)) +
 labs(title='Comparación del precio por metro cuadrado y el número de piso', x = "x", y = "Precio por metro cuadrado")+
 theme(legend.position = "bottom")   



# Condicinan Positivamente
ggplot(data = datos_4) +
 geom_mosaic(aes(x = product(precio_m2_rec, Pool), fill= precio_m2_rec)) +
 labs(title='Comparación del precio por metro cuadrado y el número de piso', x = "x", y = "Precio por metro cuadrado")+
 theme(legend.position = "bottom")   


ggplot(data = datos_4) +
 geom_mosaic(aes(x = product(precio_m2_rec, Gym), fill= precio_m2_rec)) +
 labs(title='Comparación del precio por metro cuadrado y el número de piso', x = "x", y = "Precio por metro cuadrado")+
 theme(legend.position = "bottom")   
...
ggplot(datos_4, aes(x = factor(WiFi), y = precio_m2)) +
  geom_bar(stat="identity",width = 0.2, height = 0, alpha = 0.7) +
  labs(title = "Distribución de Precio por Metro Cuadrado por Categoría",
       x = "Categoría (0 y 1)",
       y = "Precio por metro cuadrado")


max(datos_4$precio_m2)
```
...
crear_grafico_mosaico <- function(variable1) {
      ggplot(datos_4) +
    geom_mosaic(aes_string(x = sprintf("product(precio_m2_rec, %s)", variable1), fill = precio_m2_rec)) +
...
crear_grafico_mosaico(vista_al_mar)
