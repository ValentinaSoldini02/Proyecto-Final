...
# Verificamos de nuevo la cantidad de nulos
data_nulos <- data.frame(cantidad_nulos = sapply(datos, function(x) sum(is.na(x))))
# Aplicamos la función sapply par ver sumar la cantidad total de nulos en las los datos y a este objeto lo guardamos como un dataframe

data_nulos %>% filter(data_nulos > 0)
# Hacer esto en formad de función
...
datos <- datos %>% # Datos
  mutate(direccion = if_else(is.na(direccion), zona)) 
...
#   group_by(nro_de_piso) %>% #Agrupamos por número de piso
#   summarise(Cantidad = n()) # Resumimos por cantidad
# grupo_piso_1
# 
# 
# # Gastos comunes por Municipio
# ggplot(grupo_piso_1, aes(x = nro_de_piso, y=Cantidad)) +  # Datos y ejes
...
grupo_piso_1 <- datos_2 %>%
  group_by(cant_de_piso) %>%
...
# P1) Cuánto más alto el piso, más caro? Como afecta el piso en el que estoy al precio?
# P2) Cuántos más pisos tiene el inmueble (edificio), más caro es?
# P3) Cuánto más apartamentos hay en un piso, mas caro es o menos?
...
# P13) ¿Cómo influye el piso en el precio? Los pisos más altos o más bajos influyen en el precio?


Prueba11 <- datos %>%
   group_by(nro_de_piso) %>%
   summarise(media_precio = mean(precio))
Prueba11



#Prueba11[!c(102, 408, 506, 774, 802), ]
Prueba12 <- Prueba11 %>% filter(nro_de_piso < 43)



ggplot(Prueba12, aes(x=nro_de_piso, y=media_precio)) +
      geom_bar(stat="identity") +
      scale_x_continuous(breaks =  Prueba12$nro_de_piso)
...
```{r}
# P14) ¿Cómo influye la cantidad de apartamentos por piso en el precio?


grupo_piso_1 <- datos_2 %>%
  group_by(apartamentos_por_piso) %>%
  summarise(Cantidad = n())
grupo_piso_1
...
#Prueba11[!c(102, 408, 506, 774, 802), ]
#Prueba13 <- Prueba11 %>% filter(Floor < 43)

ggplot(Prueba12, aes(x=cant_de_piso, y=media_precio)) +
      geom_bar(stat="identity")
...
```{r}
# P3) Cuánto más apartamentos hay en un piso, es mas caro es o menos?
...
ggplot(Prueba14, aes(x=apartamentos_por_piso, y=media_precio)) +
      geom_bar(stat="identity")

# Los pisos con 1 apartamento son los más caros, luego si aumentamos el número de apartamentos por piso, el precio va disminuyendo
...
ggplot(Prueba13, aes(x=baños, y=media_precio)) +    geom_bar(stat="identity") + 
   scale_x_continuous(breaks =  Prueba13$baños)
...
ggplot(datos, aes(x=cant_habitaciones)) +
 geom_bar() + 
...
ggplot(Prueba15, aes(x=cant_habitaciones, y=media_precio)) +  geom_bar(stat="identity") + 
 scale_x_continuous(breaks =  Prueba15$cant_habitaciones)
...
ggplot(datos, aes(x=cant_cuartos)) +
 geom_bar() + 
...
ggplot(Prueba25, aes(x=cant_cuartos, y=media_precio)) +  geom_bar(stat="identity") + 
 scale_x_continuous(breaks =  Prueba25$cant_cuartos)
...
ggplot(datos, aes(x=cant_de_est)) +
     geom_bar() + 
...
ggplot(Prueba16, aes(x=cant_de_est, y=media_precio)) +    geom_bar(stat="identity") + 
   scale_x_continuous(breaks =  Prueba16$cant_de_est)
...
datos_2 <- datos_2 %>% filter(Built_Square_Meters < 3000 & Price < 15000)
...
ggplot(datos_2, aes(x=m2_const, y=precio)) +    geom_point()

# METROS POR TERRAZA
ggplot(datos_2, aes(x=m2_terraza, y=precio)) +     geom_point()

# METROS CUADRADOS POR TIERRA
ggplot(datos_2, aes(x=m2_totales, y=precio)) +  geom_point()
