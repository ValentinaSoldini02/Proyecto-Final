...
mean_filtrada <- datos %>%
  filter(is.finite(precio_m2)) %>%
  summarise(media = mean(precio_m2)) %>%
  pull(media)
mean_filtrada
...
datos$precio_m2[datos$precio_m2 %in% "Inf"]


sum(datos$precio_m2 %in% "Inf")
...
```{r}
# Datos nulos en direccion reemplazarlos con datos de zona
...
data_nulos_2 <- data.frame(cantidad_nulos = sapply(DMELO, function(x) sum(is.na(x))))
# Aplicamos la función sapply par ver sumar la cantidad total de nulos en las los datos y a este objeto lo guardamos como un dataframe

data_nulos_2 %>% filter(data_nulos_2 > 0)
...
ggplot(grupo_piso_1, aes(x = Floor, y=Cantidad)) +  # Datos y ejes
...
grupo_piso_1 <- datos_2 %>%
  group_by(Apartments_per_Floor) %>%
  summarise(Cantidad = n())
grupo_piso_1


# Gastos comunes por Municipio
ggplot(grupo_piso_1, aes(x = Apartments_per_Floor, y=Cantidad)) +  # Datos y ejes
   geom_histogram(stat="identity") +
#  geom_bar(stat="identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
grupo_piso_1 <- datos_2 %>%
  group_by(Number_of_Floors) %>%
  summarise(Cantidad = n())
grupo_piso_1


# Gastos comunes por Municipio
ggplot(grupo_piso_1, aes(x = Number_of_Floors, y=Cantidad)) +  # Datos y ejes
   geom_histogram(stat="identity") +
#  geom_bar(stat="identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
Prueba12 <- Prueba11 %>% filter(Floor < 43)

ggplot(Prueba12, aes(x=Floor, y=media_precio)) +
      geom_bar(stat="identity")
...
ggplot(Prueba12, aes(x=Number_of_Floors, y=media_precio)) +
      geom_bar(stat="identity")
...
ggplot(Prueba14, aes(x=Apartments_per_Floor, y=media_precio)) +
      geom_bar(stat="identity")
...
ggplot(datos, aes(x=Bathrooms)) +
        geom_bar() +
     scale_x_continuous(breaks =  Prueba13$Bathrooms)




ggplot(Prueba13, aes(x=Bathrooms, y=media_precio)) +    geom_bar(stat="identity") + 
   scale_x_continuous(breaks =  Prueba13$Bathrooms)
...
Prueba15 <- datos %>%
   group_by(Bedrooms...13) %>%
   summarise(media_precio = mean(Price))
Prueba15



ggplot(datos, aes(x=Bedrooms...13)) +
     geom_bar() + 
     scale_x_continuous(breaks =  Prueba15$Bedrooms...13)




ggplot(Prueba15, aes(x=Bedrooms...13, y=media_precio)) +    geom_bar(stat="identity") + 
   scale_x_continuous(breaks =  Prueba15$Bedrooms...13)
...
ggplot(datos, aes(x=Garages)) +
     geom_bar() + 
     scale_x_continuous(breaks =  Prueba16$Garages)




ggplot(Prueba16, aes(x=Garages, y=media_precio)) +    geom_bar(stat="identity") + 
   scale_x_continuous(breaks =  Prueba16$Garages)
...
ggplot(datos_2, aes(x=Built_Square_Meters, y=Price)) +    geom_point()

# METROS POR TERRAZA
ggplot(datos_2, aes(x=Terrace_Square_Meters, y=Price)) +     geom_point()

# METROS CUADRADOS POR TIERRA
ggplot(datos_2, aes(x=Land_Square_Meters, y=Price)) +  geom_point()
