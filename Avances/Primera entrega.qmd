...
```{r}
# Función para columnas de ceros y unos
grafico_ceros_unos <- function(datos, columna){

  grupillo <- datos %>%
    group_by({{ columna }}) %>%
    summarise(conteo = mean(Price))

  ggplot(grupillo, aes(x = {{ columna }}, y = conteo)) + 
    geom_bar(stat = "identity")
}
...
grafico_ceros_unos(datos_2, Sea_View)
grafico_ceros_unos(datos_2, Barbecue)
grafico_ceros_unos(datos_2, Closet)
grafico_ceros_unos(datos_2, Pool)
grafico_ceros_unos(datos_2, Pet)


 [1] "Sea_View"              "Social_Housing"       
 [3] "Barbecue"              "Living_Room"          
 [5] "Closet"                "Patio"                
 [7] "Pool"                  "Garage"               
 [9] "Pet"                   "Gas_Pipeline"         
[11] "Playroom"              "Duplex"               
...
[15] "Grill"                 "Gym"                  
[17] "Balcony"               "Storage"              
[19] "Air_Conditioning"      "Garden"               
[21] "Radiant_Floor_Heating" "Solarium"  
...
grafico_ceros_unos <- function(datos) {
  # Verifica que todas las columnas del dataframe sean binarias (0 o 1)


  # Lista para almacenar los gráficos
  lista_graficos <- list()
  
  # Genera un gráfico de barras para cada columna binaria
   for (columna in names(datos)) {
    grupillo <- datos %>%
      group_by(.data[[columna]]) %>%
      summarise(conteo = mean(datos_2$Price))
    
    grafico <- ggplot(grupillo, aes(x = .data[[columna]], y = conteo)) + 
      geom_bar(stat = "identity") +
      ggtitle(paste("Gráfico de", columna))
    
    lista_graficos[[columna]] <- grafico
  }
  
  # Muestra todos los gráficos en una grilla
  do.call(grid.arrange, c(lista_graficos, ncol = 3))#, nrow=))
}
...
grafico_ceros_unos <- function(datos, ncol = 2, nrow = 2) {
  # Verifica que todas las columnas del dataframe sean binarias (0 o 1)
  columnas_binarias <- datos %>%
  ...
  for (columna in names(datos)) {
    grupillo <- datos %>%
      group_by(.data[[columna]]) %>%
      summarise(conteo = mean(data.frame(datos_2$Price)))
  ...
  # Dividir gráficos en páginas
  num_graficos <- length(lista_graficos)
  num_paginas <- ceiling(num_graficos / (ncol * nrow))
  
  for (i in seq_len(num_paginas)) {
    start_index <- (i - 1) * (ncol * nrow) + 1
    end_index <- min(i * (ncol * nrow), num_graficos)
    graficos_pagina <- lista_graficos[start_index:end_index]
    
    do.call(grid.arrange, c(graficos_pagina, ncol = ncol))
  }
}
...
grafico_ceros_unos(dum, ncol = 2, nrow = 2)
