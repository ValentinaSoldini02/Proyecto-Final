...
# diego
prueba_22 <- prueba_2 %>% filter(Price <= 150000 & Common_Expenses <= 10000)
...
ggplot(data = prueba_22, aes(x = Zone, y = Price, fill = Municipio)) + 
...
# Crear el gráfico de barras con colores personalizados
ggplot(data = prueba_22, aes(x = Property_Type, y = Price, fill = Municipio)) + 
  geom_bar(stat = "identity", position = "dodge", width = 0.7) + # Ajustar el ancho de las barras
  facet_grid(. ~ Municipio, scales = "free_x") + # Usar facet_grid para crear una grilla por Municipio
  labs(x = "Zona", y = "Precio", fill = "Municipio") + # Etiquetas de los ejes y la leyenda
  ggtitle("Precio por Zona y Municipio") + # Título del gráfico
...
```{r}
# Pregunta 3) Que tipos de alquileres hay?
ggplot(prueba_2, aes(x = Property_Type)) +
  geom_bar() +
  theme(axis.text.x = element_text(size=7))
...
# Pregunta 3) En que estado se encuentran la mayor de los alquileres por municipio?


# Pregunta 4) Como están distribuidos el tipo de alquiler por municipio
ggplot(datos_tipo, aes(x = Municipio, y = nueva, fill = Property_Type)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust = 1, size = 8)) +
  labs(x = "Municipio", y = "Cantidad", fill = "Tipo de Propiedad") +
...
```{r}
# La mayoría de alquileres son apartamentos
prueba_22 %>% filter(Common_Expenses < 10000)

```

```{r}

ggplot(prueba_22, aes(x = Municipio, y = Common_Expenses)) +
  geom_bar(stat="identity") +
  theme(axis.text.x = element_text(size=7))
```
...
ggplot(prueba_22, aes(x = Condition)) +
  geom_bar() +
  theme(axis.text.x = element_text(size=7))
```
...
ggplot(prueba_22, aes(x = Municipio, fill = Condition)) +
...
# La mayoría de viviendas que están en buenas
# o excelentes condiciones están en el municipio CH
# dónde también están las viviendas más caras




# Crear el gráfico de barras con colores personalizados
ggplot(data = prueba_22, aes(x = Condition, y = Price, fill = Municipio)) + 
  geom_bar(stat = "identity", position = "dodge", width = 0.7) + # Ajustar el ancho de las barras
  facet_grid(. ~ Municipio, scales = "free_x") + # Usar facet_grid para crear una grilla por Municipio
  labs(x = "Zona", y = "Precio", fill = "Municipio") + # Etiquetas de los ejes y la leyenda
  ggtitle("Precio por Zona y Municipio") + # Título del gráfico
...
# La gran mayoría son apartamentos
...
#  En que condición se encuentran la mayoría de las viviendas por municipio?

#  Qué tipo de inmuebles hay? (oficinas, casas, apartamentos)

#  En que estado se encuentran la mayor de los alquileres por municipio?
...
P3)  En que condición se encuentran la mayoría de las viviendas por municipio?

P4)  Qué tipo de inmuebles hay? (oficinas, casas, apartamentos)

P5)  En que estado se encuentran la mayor de los alquileres por municipio?
...
# Datos sobre precios de alquileres
datos <- read_csv(here("houses.csv"))
datos
...
El criterio de rango intercuartilico dice que si un valor esta por debajo del limite inferior (que es el quantil Q1 -1,5 * IQR) o por encima del limite superior (que es el quantil Q3 + 1,5 * IQR)
...
# Pregunta 4) ¿En que estados se encuentran los alquileres?
datos_2
...
```{r}
# Pregunta 2) Que tipos de alquileres hay?
ggplot(prueba_2, aes(x = Property_Type)) +
  geom_bar() +
  theme(axis.text.x = element_text(size=7))
...
# P3)  Qué tipo de inmuebles hay? (oficinas, casas, apartamentos)
